...
const SingleCard = ({card, handleWantToCook}) => {
    ...
    return (
        <div className="border-2 border-gray-200 p-5 space-y-5 rounded-3xl  text-gray-500">
            ...
            <div>
                {
                ingredients.map((item, idx) => <Ingredients item = {item} key = {idx}></Ingredients>)
            }
            </div>
            <hr />
            ...
            <button onClick={() => handleWantToCook(card)} className="btn rounded-full px-5 border-0 mx-4 font-semibold text-xl bg-[#0BE58A]">Want to Cook</button>
